...
public class ContainerEnchantment extends Container
{
    ...
    {
        this(p_i45797_1_, p_i45797_2_, BlockPos.ZERO);
    }
    ...
    public ContainerEnchantment(InventoryPlayer p_i45798_1_, World p_i45798_2_, BlockPos p_i45798_3_)
    {
        ...
        this.addSlot(new Slot(this.tableInventory, 1, 35, 47)
        {
            public boolean isItemValid(ItemStack stack)
            {
                return stack.getItem() == Items.field_151100_aR && EnumDyeColor.func_176766_a(stack.func_77960_j()) == EnumDyeColor.BLUE;
            }
        });
    ...
    public void onCraftMatrixChanged(IInventory inventoryIn)
    {
        if (inventoryIn == this.tableInventory)
        {
            ...
            {
                if (!this.field_75172_h.isRemote)
                {
                    int l = 0;
                    ...
                    for (int j = -1; j <= 1; ++j)
                    {
                        for (int k = -1; k <= 1; ++k)
                        {
                            if ((j != 0 || k != 0) && this.field_75172_h.isAirBlock(this.field_178150_j.add(k, 0, j)) && this.field_75172_h.isAirBlock(this.field_178150_j.add(k, 1, j)))
                            {
                                if (this.field_75172_h.getBlockState(this.field_178150_j.add(k * 2, 0, j * 2)).getBlock() == Blocks.BOOKSHELF)
                                {
                                    ++l;
                                }

                                if (this.field_75172_h.getBlockState(this.field_178150_j.add(k * 2, 1, j * 2)).getBlock() == Blocks.BOOKSHELF)
                                {
                                    ++l;
                                }
                                ...
                                if (k != 0 && j != 0)
                                {
                                    if (this.field_75172_h.getBlockState(this.field_178150_j.add(k * 2, 0, j)).getBlock() == Blocks.BOOKSHELF)
                                    {
                                        ++l;
                                    }

                                    if (this.field_75172_h.getBlockState(this.field_178150_j.add(k * 2, 1, j)).getBlock() == Blocks.BOOKSHELF)
                                    {
                                        ++l;
                                    }

                                    if (this.field_75172_h.getBlockState(this.field_178150_j.add(k, 0, j * 2)).getBlock() == Blocks.BOOKSHELF)
                                    {
                                        ++l;
                                    }

                                    if (this.field_75172_h.getBlockState(this.field_178150_j.add(k, 1, j * 2)).getBlock() == Blocks.BOOKSHELF)
                                    {
                                        ++l;
                                    }
                                }
                    ...
                    for (int j1 = 0; j1 < 3; ++j1)
                    {
                        if (this.enchantLevels[j1] > 0)
                        {
                            ...
                            if (list != null && !list.isEmpty())
                            {
                                ...
                                this.enchantClue[j1] = Enchantment.func_185258_b(enchantmentdata.enchantment);
        ...
        else if (this.enchantLevels[id] > 0 && !itemstack.isEmpty() && (playerIn.experienceLevel >= i && playerIn.experienceLevel >= this.enchantLevels[id] || playerIn.abilities.isCreativeMode))
        {
            if (!this.field_75172_h.isRemote)
            {
                ...
                if (!list.isEmpty())
                {
                    ...
                    this.field_75172_h.playSound((EntityPlayer)null, this.field_178150_j, SoundEvents.BLOCK_ENCHANTMENT_TABLE_USE, SoundCategory.BLOCKS, 1.0F, this.field_75172_h.rand.nextFloat() * 0.1F + 0.9F);
                }
            }
    ...
    {
        if (this.field_75172_h.getBlockState(this.field_178150_j).getBlock() != Blocks.ENCHANTING_TABLE)
        {
            return false;
        }
        else
        {
            return playerIn.getDistanceSq((double)this.field_178150_j.getX() + 0.5D, (double)this.field_178150_j.getY() + 0.5D, (double)this.field_178150_j.getZ() + 0.5D) <= 64.0D;
        }
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            else if (itemstack1.getItem() == Items.field_151100_aR && EnumDyeColor.func_176766_a(itemstack1.func_77960_j()) == EnumDyeColor.BLUE)
            {
            ...
            else
            {
                ...
                {
                    ((Slot)this.inventorySlots.get(0)).putStack(new ItemStack(itemstack1.getItem(), 1, itemstack1.func_77960_j()));
                    itemstack1.shrink(1);
                }
            }
