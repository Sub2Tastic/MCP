...
public class EntityLightningBolt extends EntityWeatherEffect
{
    ...
    public EntityLightningBolt(World worldIn, double x, double y, double z, boolean effectOnlyIn)
    {
        ...
        if (!effectOnlyIn && !worldIn.isRemote && worldIn.getGameRules().func_82766_b("doFireTick") && (worldIn.getDifficulty() == EnumDifficulty.NORMAL || worldIn.getDifficulty() == EnumDifficulty.HARD) && worldIn.func_175697_a(blockpos, 10))
        {
            if (worldIn.getBlockState(blockpos).getMaterial() == Material.AIR && Blocks.FIRE.func_176196_c(worldIn, blockpos))
            {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                if (worldIn.getBlockState(blockpos1).getMaterial() == Material.AIR && Blocks.FIRE.func_176196_c(worldIn, blockpos1))
                {
        ...
        if (this.lightningState < 0)
        {
            ...
            else if (this.lightningState < -this.rand.nextInt(10))
            {
                ...
                if (!this.effectOnly && !this.world.isRemote)
                {
                    ...
                    if (this.world.getGameRules().func_82766_b("doFireTick") && this.world.func_175697_a(blockpos, 10) && this.world.getBlockState(blockpos).getMaterial() == Material.AIR && Blocks.FIRE.func_176196_c(this.world, blockpos))
                    {
