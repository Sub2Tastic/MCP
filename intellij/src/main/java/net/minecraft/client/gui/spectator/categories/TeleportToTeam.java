...
public class TeleportToTeam implements ISpectatorMenuView, ISpectatorMenuObject
{
    ...
    class TeamSelectionObject implements ISpectatorMenuObject
    {
        ...
        {
            int i = -1;
            String s = FontRenderer.func_78282_e(this.team.func_96668_e());

            if (s.length() >= 2)
            {
                i = Minecraft.getInstance().fontRenderer.func_175064_b(s.charAt(1));
            }
            ...
            if (i >= 0)
            {
                ...
                Gui.func_73734_a(1, 1, 15, 15, MathHelper.rgb(f * brightness, f1 * brightness, f2 * brightness) | alpha << 24);
            }

            Minecraft.getInstance().getTextureManager().bindTexture(this.location);
            GlStateManager.func_179131_c(brightness, brightness, brightness, (float)alpha / 255.0F);
