...
public class AdvancementToast implements IToast
{
    ...
    public IToast.Visibility draw(GuiToast toastGui, long delta)
    {
        ...
        if (displayinfo != null)
        {
            ...
            if (list.size() == 1)
            {
                toastGui.getMinecraft().fontRenderer.func_78276_b(I18n.format("advancements.toast." + displayinfo.getFrame().getName()), 30, 7, i | -16777216);
                toastGui.getMinecraft().fontRenderer.func_78276_b(displayinfo.getTitle().getFormattedText(), 30, 18, -1);
            }
            else
            {
                int j = 1500;
                ...
                if (delta < 1500L)
                {
                    ...
                    toastGui.getMinecraft().fontRenderer.func_78276_b(I18n.format("advancements.toast." + displayinfo.getFrame().getName()), 30, 11, i | k);
                }
                else
                {
                    ...
                    for (String s : list)
                    {
                        toastGui.getMinecraft().fontRenderer.func_78276_b(s, 30, l, 16777215 | i1);
