...
public class Selector
{
    ...
    {
        return this.condition.func_188118_a(state);
    }
    ...
    public static class Deserializer implements JsonDeserializer<Selector>
    {
        ...
        {
            return json.has("when") ? getOrAndCondition(JsonUtils.getJsonObject(json, "when")) : ICondition.field_188119_a;
        }
        ...
        static ICondition getOrAndCondition(JsonObject json)
        {
            ...
            else if (set.size() == 1)
            {
                if (json.has("OR"))
                {
                    return new ConditionOr(Iterables.transform(JsonUtils.getJsonArray(json, "OR"), field_188163_a));
                }
                else
                {
                    return (ICondition)(json.has("AND") ? new ConditionAnd(Iterables.transform(JsonUtils.getJsonArray(json, "AND"), field_188163_a)) : makePropertyValue(set.iterator().next()));
                }
            }
            else
            {
