...
public class MapData extends WorldSavedData
{
    ...
    public void updateVisiblePlayers(EntityPlayer player, ItemStack mapStack)
    {
        ...
        for (int i = 0; i < this.playersArrayList.size(); ++i)
        {
            ...
            if (!mapdata$mapinfo1.player.removed && (mapdata$mapinfo1.player.inventory.hasItemStack(mapStack) || mapStack.isOnItemFrame()))
            {
                ...
                {
                    this.updateDecorations(MapDecoration.Type.PLAYER, mapdata$mapinfo1.player.world, mapdata$mapinfo1.player.func_70005_c_(), mapdata$mapinfo1.player.posX, mapdata$mapinfo1.player.posZ, (double)mapdata$mapinfo1.player.rotationYaw);
                }
            }
            else
            {
    ...
    public class MapInfo
    {
        ...
        public Packet<?> getPacket(ItemStack stack)
        {
            ...
            {
                this.isDirty = false;
                return new SPacketMaps(stack.func_77960_j(), MapData.this.scale, MapData.this.trackingPosition, MapData.this.mapDecorations.values(), MapData.this.colors, this.minX, this.minY, this.maxX + 1 - this.minX, this.maxY + 1 - this.minY);
            }
            else
            {
                return this.tick++ % 5 == 0 ? new SPacketMaps(stack.func_77960_j(), MapData.this.scale, MapData.this.trackingPosition, MapData.this.mapDecorations.values(), MapData.this.colors, 0, 0, 0, 0) : null;
            }
        }
