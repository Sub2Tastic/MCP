...
public class WorldGenBigTree extends WorldGenAbstractTree
{
    ...
    private BlockPos field_175947_m = BlockPos.ZERO;
    int field_76504_e;
    int field_76501_f;
        ...
        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_181631_2_ * p_181631_2_))
                {
                    ...
                    Material material = this.field_175946_l.getBlockState(blockpos).getMaterial();
    ...
    void func_175940_a(BlockPos p_175940_1_)
    {
        ...
        {
            this.func_181631_a(p_175940_1_.up(i), this.func_76495_b(i), Blocks.field_150362_t.getDefaultState().func_177226_a(BlockLeaves.field_176236_b, Boolean.valueOf(false)));
        }
    }
    ...
    int func_175936_a(BlockPos p_175936_1_, BlockPos p_175936_2_)
    {
        ...
        else
        {
            for (int j = 0; j <= i; ++j)
            {
                ...
                if (!this.func_150523_a(this.field_175946_l.getBlockState(blockpos1).getBlock()))
                {
                    return j;
                }
            }
    ...
    {
        Block block = this.field_175946_l.getBlockState(this.field_175947_m.down()).getBlock();
