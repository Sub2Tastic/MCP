    ...
    public boolean load(boolean requireMatchingSize)
    {
        if (this.mode == TileEntityStructure.Mode.LOAD && !this.world.isRemote && !StringUtils.isNullOrEmpty(this.name))
        {
            ...
            else
            {
                ...
                {
                    PlacementSettings placementsettings = (new PlacementSettings()).setMirror(this.mirror).setRotation(this.rotation).setIgnoreEntities(this.ignoreEntities).setChunk((ChunkPos)null).func_186225_a((Block)null).func_186226_b(false);

                    if (this.integrity < 1.0F)
                    {
                        placementsettings.func_189946_a(MathHelper.clamp(this.integrity, 0.0F, 1.0F)).func_189949_a(Long.valueOf(this.seed));
                    }
